...
(defn flash-div [flash]
  [:div {:class
         (str
          ...
          " "
          (tw! " w-1/2 mt-6 mx-auto border px-4 py-3 rounded relative"))
         :role "alert"}
   (:msg flash)
...
(defn header [is-admin?]
  [:nav {:class (tw! "w-full flex items-center justify-between flex-wrap bg-klgreen-100 p-6")}
   ...
   (if is-admin?
     (let [login-status [:span {:class (tw! "ml-auto mr-6 rounded text-teal-200 border-teal-400")}
                         (str "Logged in as : " is-admin?)]
           logout [:div {:class (tw! "flex-shrink-0 text-white mr-6")}
                   [:a {:href (rfe/href :kemplittle.routes/admin)}
                    [:button {:class (tw! "px-3 py-2 border rounded text-teal-200 border-teal-400 hover:text-white hover:border-white")
                              :on-click #(do
                                           (xf/dispatch [:log-out]))}
                     [:span "Log out"]]]]
           menu [:div {:class (tw! "block lg:hidden")}
                 [:button {:class (tw! "px-3 py-2 border rounded text-teal-200 border-teal-400 hover:text-white hover:border-white")}
           ...
           server-logs-btn [:a {:href (rfe/href :kemplittle.routes/admin-logs)}
